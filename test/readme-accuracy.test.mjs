...
async function introspect() {
  const transport = new StdioClientTransport({
    command: "node",
    args: ["dist/cli.js"],
    env: { ...process.env, GA4_CLIENT_ID: "i", GA4_CLIENT_SECRET: "s", GA4_REFRESH_TOKEN: "r" },
  });
...
test("README documents every tool the server exposes", async () => {
  ...
  const cited = new Set([...README.matchAll(/`(ga4_[a-z_]+)`/g)].map((m) => m[1]));
  // Grouped rows such as `ga4_rename_campaign` / `_adset` / `_ad`
...
test("README documents every resource the server exposes", async () => {
  ...
  const cited = new Set([...README.matchAll(/`(ga4:\/\/[a-z0-9-]+)`/g)].map((m) => m[1]));
...
test("README tool counts match reality", async () => {
  ...
  assert.equal(tools.length, 27, "tool count changed, update the README");
  assert.equal(resources.length, 6, "resource count changed, update the README");

  assert.match(README, /\*\*27 read tools\*\*/);
});
